...
export default async function handler(req, res) {
  ...
  const { share_code, new_email, member_id, current_email } = req.body || {};
  ...
  try {
    ...
    if (member_id) {
      ...
      if (!ref) {
        return res
          .status(403)
          .json({ error: "That member is not in your squad." });
      }
      targetId = member_id;
    } else {
      targetId = owner.id;
    ...
    if (current_email && typeof current_email === "string") {
      ...
      if (targetUser.email !== normalizedCurrent) {
        return res
          .status(403)
          .json({ error: "You can only edit your own email." });
      }
    }
